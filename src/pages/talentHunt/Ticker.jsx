...
export default function Ticker() {
  ...
  return (
    <div className="w-full overflow-hidden flex items-center relative border-b"
      ...
      {/* Edge fades */}
      <div className="absolute left-0 z-10 w-16 h-full pointer-events-none"
        style={{ background: "linear-gradient(to right, #091749, transparent)" }} />
      <div className="absolute right-0 z-10 w-16 h-full pointer-events-none"
        style={{ background: "linear-gradient(to left, #091749, transparent)" }} />

      <motion.div className="flex whitespace-nowrap"
        animate={{ x: ["0%", "-50%"] }}
        transition={{ repeat: Infinity, ease: "linear", duration: 40 }}>
        {items.map((item, i) => (
          <span key={i} className="inline-flex items-center text-[11px] font-semibold px-6"
            style={{ color: "#f0d060" }}>
            {item}

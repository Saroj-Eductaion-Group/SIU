...
const SUBJECTS = [
  { name: "Physics",       color: "#dbeafe", text: "#1d4ed8" },
  { name: "Chemistry",     color: "#dcfce7", text: "#15803d" },
  { name: "Biology",       color: "#d1fae5", text: "#065f46" },
  { name: "Mathematics",   color: "#fee2e2", text: "#b91c1c" },
  { name: "English",       color: "#f3e8ff", text: "#7c3aed" },
  { name: "General Test",  color: "#fef3c7", text: "#92400e" },
  { name: "Reasoning",     color: "#e0e7ff", text: "#3730a3" },
  { name: "Economics",     color: "#fce7f3", text: "#9d174d" },
];

const PARTNERS = ["SIU Lucknow","CBSE","UP Board","Narayana IIT","Resonance","FIITJEE","Aakash","Allen Kota"];

export default function HomePanel({ setActiveTab, onScholarship }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="relative rounded-2xl overflow-hidden shadow-2xl"
        style={{ background: "linear-gradient(145deg, #0b1f5e 0%, #131f4f 40%, #0a1435 100%)", minHeight: 340 }}>
        {/* Background mesh */}
          ...
          <div className="flex flex-col lg:flex-row gap-8 items-start">
            ...
            <div className="flex-1 min-w-0">
              ...
              <h1 className="font-black leading-tight mb-3"
                style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(26px, 4vw, 44px)", color: "#ffffff" }}>
                Practice Smart.<br />
              ...
              <div className="flex gap-3 flex-wrap mb-6">
                <button onClick={() => setActiveTab("mocks")}
                  className="flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold transition hover:brightness-105 active:scale-95 shadow-lg"
              ...
              <div key={s.num} className="rounded-xl px-4 py-3 flex items-center gap-3"
                ...
                <div>
                  <div className="font-black text-lg leading-none"
                    style={{ color: s.color, fontFamily: "'Playfair Display', serif" }}>{s.num}</div>
                  <div className="text-[10px] mt-0.5 uppercase tracking-wider"
                    style={{ color: "rgba(255,255,255,0.42)" }}>{s.label}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-black text-lg text-gray-900"
            style={{ fontFamily: "'Playfair Display', serif" }}>Everything You Need to Crack CUET</h2>
          <span className="text-xs text-gray-400 hidden sm:block">All features are free for registered candidates</span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {FEATURES.map(f => (
            <button key={f.title} onClick={() => setActiveTab(f.tab)}
              ...
              <span className="inline-block px-2.5 py-1 rounded-lg text-xs font-bold text-white"
      ...
      {/* ── WHY US ── */}
      <div className="rounded-2xl px-6 md:px-8 py-7"
        style={{ background: "linear-gradient(135deg, #0b1f5e 0%, #1a1060 100%)", boxShadow: "0 4px 24px rgba(10,31,92,0.25)" }}>
        <div className="text-center mb-5">
          ...
          <h2 className="font-black text-white text-xl mt-1"
            style={{ fontFamily: "'Playfair Display', serif" }}>Built for Serious CUET Aspirants</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {WHY_US.map(w => (
            <div key={w.title} className="rounded-xl p-4 text-center"
              style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)" }}>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl px-6 py-5 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-black text-gray-900 text-base"
            style={{ fontFamily: "'Playfair Display', serif" }}>Subjects Covered</h2>
          <button onClick={() => setActiveTab("mocks")}
            className="text-xs font-semibold hover:text-purple-800 transition" style={{ color: "#7c3aed" }}>
            View all tests →
          </button>
        </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl px-5 py-4 shadow-sm">
        ...
        <div className="flex flex-wrap gap-1.5">
          {PARTNERS.map(p => (
            <span key={p} className="bg-gray-50 border border-gray-200 rounded-lg px-2.5 py-1.5 text-xs font-semibold"
              style={{ color: "#0a1f5c" }}>{p}</span>
          ))}
        </div>
      </div>
        ...
        <div className="px-6 md:px-10 py-7 flex flex-col md:flex-row items-center gap-5">
          ...
          <div className="text-white flex-1 text-center md:text-left">
            <div className="font-black text-2xl mb-1"
              style={{ fontFamily: "'Playfair Display', serif" }}>Win ₹51,000 Scholarship!</div>
            <div className="text-sm opacity-80 max-w-lg">
              Register for SIUAT Talent Hunt Exam 2026-27. Score 90%+ to unlock a 100% full scholarship at Saroj International University, Lucknow.
            </div>
          </div>
          <div className="flex flex-col gap-2 items-center flex-shrink-0">
            <button onClick={onScholarship}
              className="font-extrabold text-sm px-6 py-3 rounded-xl transition hover:brightness-105 active:scale-95"

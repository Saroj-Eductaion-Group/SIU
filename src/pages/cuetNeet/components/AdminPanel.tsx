...
export function AdminPanel() {
  ...
  const login = () => {
    ...
    if (pwInput === adminPass) {
      ...
      setLoginError(""); setPwInput("");
    } else {
      const newAttempts = attempts + 1;
      setAttempts(newAttempts);
      if (newAttempts >= 5) { setLocked(true); setLoginError("Too many failed attempts. Admin panel locked for this session."); }
      else setLoginError(`Invalid password. ${5 - newAttempts} attempt(s) remaining.`);
    }
  };

  const logout = () => { setLoggedIn(false); sessionStorage.removeItem("admin_in"); };
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <button onClick={() => setRegistrations(SEEDED_REGISTRATIONS)} className="text-xs px-3 py-2 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-50">↺ Refresh</button>
